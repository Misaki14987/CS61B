...
public class Model {
     ...
     */
    public boolean isTwoValueEqual(int x, int y) {
        return x == y;
    }
    ...
    public boolean atLeastOneMoveExists() {
        for (int i = 0; i < size(); i++) {
            for (int j = 0; j < size(); j++) {
                ...
                if (i != 0 && tile(i - 1, j) != null && isTwoValueEqual(tile(i, j).value(), tile(i - 1, j).value())) {
                    return true;
                }
                if (i != size() - 1 && tile(i + 1, j) != null && isTwoValueEqual(tile(i, j).value(), tile(i + 1, j).value())) {
                    return true;
                }
                if (j != size() - 1 && tile(i, j + 1) != null && isTwoValueEqual(tile(i, j).value(), tile(i, j + 1).value())) {
                    return true;
                }
                if (j != 0 && tile(i, j - 1) != null && isTwoValueEqual(tile(i, j).value(), tile(i, j - 1).value())) {
                    return true;
                }
            }
        }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        int targetY = y;
        for (; targetY < size() - 1; targetY++) {
            if (tile(x, targetY + 1) != null && (myValue != tile(x, targetY + 1).value()
                    || tile(x, targetY + 1).wasMerged())) {
                break;
            }
        }
        if (tile(x, targetY) != currTile) {
